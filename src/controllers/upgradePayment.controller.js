 ...
 * Public — serves a real HTTPS HTML page that opens Razorpay checkout.js.
 * Using a real URL (not inline data) avoids WebView2 cross-origin iframe restrictions.
...
<body>
  <div class="loader">
    ...
    <p>Opening payment gateway&hellip;</p>
  ...
  <script>
    function sendResult(data) {
      if (window.flutter_inappwebview) {
        window.flutter_inappwebview.callHandler('paymentResult', JSON.stringify(data));
      }
    }
    function openRazorpay() {
      var options = {
        ...
        theme:       { color: '#1a1a3e' },
        modal: {
          ondismiss: function() { sendResult({ status: 'dismissed' }); }
        },
      ...
      try {
        ...
        rzp.on('payment.failed', function(r) {
          sendResult({ status: 'failed', error_code: r.error.code, description: r.error.description });
        });
        rzp.open();
      } catch(e) {
        document.getElementById('errMsg').textContent = 'Error: ' + e.message;
        sendResult({ status: 'failed', description: 'Checkout init failed: ' + e.message });
      }
    }
