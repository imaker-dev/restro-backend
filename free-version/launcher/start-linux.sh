#!/bin/bash
# Restro POS - Free Offline Version (Linux Launcher)
# Same as macOS launcher — Linux uses identical MariaDB commands
...
if [ -f "$MARIADB_DIR/bin/mariadbd" ]; then
    ...
    echo "Download it first: node free-version/scripts/download-mariadb.js --platform=linux"
    exit 1
fi
...
if [ ! -f "$DATA_DIR/.initialized" ]; then
    ...
    if [ -n "$INSTALL_DB" ]; then
        "$INSTALL_DB" --datadir="$DATA_DIR" --basedir="$MARIADB_DIR" --auth-root-authentication-method=normal
    else
