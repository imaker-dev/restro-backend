/**
 * IMPORT Apr 9 orders from Excel into DB (outlet 46)
 * 
 * Rules (same as Apr 8):
 *   - Exclude Swiggy/Online orders
 *   - Zomato orders → imported as outside_collections (not regular orders)
 *     with description "zomato" in Shiv's shift (floor 39)
 ...
 *   - Create day_sessions (shifts) for Apr 9
 *   - Zomato outside_collections linked to Shiv's shift
 *   - FULL TRANSACTION: rollback on ANY error
 * 
 * Usage:
 *   node tests/import-apr9-orders.js              # DRY RUN
 *   node tests/import-apr9-orders.js --execute     # ACTUAL IMPORT
 */
...
const OUTLET_ID = 46;
const file = path.join(__dirname, '..', 'scripts', '9thApril-Order.xlsx');
...
async function run() {
  ...
  // Separate Zomato/Swiggy/Online from importable
  const zomatoOrders = [];
  ...
  for (const [no, rows] of Object.entries(groups)) {
    ...
    if (sub.includes('zomato')) {
      zomatoOrders.push({ no, main, rows });
      excludedNos.add(no);
    } else if (sub.includes('swiggy') || pay === 'online') {
      excludedNos.add(no);
    }
  }
  ...
  console.log(`Excluded (Swiggy/Online non-Zomato): ${excludedNos.size - zomatoOrders.length} orders`);
  console.log(`Zomato orders (→ outside_collections): ${zomatoOrders.length} orders`);
  console.log(`Importable (regular): ${importableGroups.length} orders`);
  ...
  const [existingOrders] = await pool.query(
    `SELECT order_number FROM orders WHERE outlet_id = ? AND order_number LIKE '${ORDER_PREFIX}%' ORDER BY order_number DESC LIMIT 1`, [OUTLET_ID]
  );
  let nextSeq = 1;
  ...
  const [existingPay] = await pool.query(
    `SELECT payment_number FROM payments WHERE outlet_id = ? AND payment_number LIKE '${PAY_PREFIX}%' ORDER BY payment_number DESC LIMIT 1`, [OUTLET_ID]
  );
  let nextPaySeq = 1;
  ...
  console.log(`Starting: ${ORDER_PREFIX}${String(nextSeq).padStart(4, '0')}, ${PAY_PREFIX}${String(nextPaySeq).padStart(4, '0')}\n`);
  ...
  for (const [excelOrderNo, rows] of importableGroups) {
      ...
      for (const [name, qty] of Object.entries(countMap)) {
        ...
        if (variantMap[key]) {
          ...
          itemInfo = {
            ...
            variant_name: null, unit_price: 0,
    ...
    const orderNumber = `${ORDER_PREFIX}${String(nextSeq).padStart(4, '0')}`;
    nextSeq++;
  ...
  // ══════════════════════════════════════════════════════════════
  // 3b. PARSE ZOMATO ORDERS → outside_collections
  // ══════════════════════════════════════════════════════════════
  const outsideCollections = [];
  for (const z of zomatoOrders) {
    const main = z.main;
    const grandTotal = r2(main[gtCol]);
    const created = String(main['Created'] || '').trim();
    const createdDate = new Date(created);
    if (isNaN(createdDate.getTime())) {
      errors.push(`Zomato #${z.no}: Cannot parse date "${created}"`);
      continue;
    }
    const createdAt = `${createdDate.getFullYear()}-${String(createdDate.getMonth()+1).padStart(2,'0')}-${String(createdDate.getDate()).padStart(2,'0')} ${String(createdDate.getHours()).padStart(2,'0')}:${String(createdDate.getMinutes()).padStart(2,'0')}:${String(createdDate.getSeconds()).padStart(2,'0')}`;
    const itemsStr = String(main['Items'] || '').trim();

    outsideCollections.push({
      excelNo: z.no,
      amount: grandTotal,
      payment_mode: 'other',  // Zomato settles via their platform
      reason: 'Zomato Order',
      description: `zomato - Order #${z.no} - ${itemsStr}`,
      collection_date: DATE_STR,
      collected_by: 205,  // Shiv
      floor_id: 39,       // Third Floor (Shiv's floor)
      createdAt
    });
  }
  ...
  for (const o of ordersToInsert) {
    ...
    console.log(`    ${o.orderNumber} | Excel#${o.excelOrderNo} | ${(o.tableName || 'PICKUP').padEnd(6)} | Rs ${String(o.totalAmount).padStart(8)} | ${payInfo.padEnd(15)} | ${o.paymentStatus.padEnd(9)} | ${o.createdAt} | ${itemCount} items`);
  }

  // Zomato outside collections
  console.log('\n' + '═'.repeat(100));
  console.log('  ZOMATO → OUTSIDE COLLECTIONS (in Shiv\'s shift)');
  console.log('═'.repeat(100));
  let totalZomato = 0;
  for (const oc of outsideCollections) {
    totalZomato += oc.amount;
    console.log(`    Excel#${oc.excelNo} | Rs ${oc.amount} | ${oc.description.substring(0, 70)}`);
  }
  console.log(`  Total Zomato outside_collections: ${outsideCollections.length}, Rs ${r2(totalZomato)}`);

  if (!EXECUTE) {
    console.log('\n' + '═'.repeat(100));
    console.log('  🔍 DRY RUN COMPLETE — No changes made to DB');
    console.log('  To execute: node tests/import-apr9-orders.js --execute');
    console.log('═'.repeat(100));
    await pool.end();
    return;
  }
  ...
  try {
    ...
    for (const o of ordersToInsert) {
      ...
      for (const pay of o.payments) {
        const payUuid = uuid();
        const payNumber = `${PAY_PREFIX}${String(paySeq).padStart(4, '0')}`;
        paySeq++;
    ...
    // ══════════════════════════════════════════════════════════════
    // 5c. Create day_sessions (shifts) for Apr 9
    // ══════════════════════════════════════════════════════════════
    console.log('\n  Creating day_sessions for Apr 9...');
    ...
    if (barOrders.length > 0) {
      ...
      shifts.push({
        floor_id: 38, cashier_id: 201, session_date: DATE_STR,
    ...
    if (restOrders.length > 0) {
      ...
      shifts.push({
        floor_id: 39, cashier_id: 205, session_date: DATE_STR,
    ...
    let shivShiftId = null; // Track Shiv's shift for outside_collections
    ...
    for (const sh of shifts) {
      ...
      const fl = sh.floor_id === 38 ? 'BAR (Aditya)' : 'REST (Shiv)';
      console.log(`    Shift #${shResult.insertId} | ${fl} | ${sh.opening_time} to ${sh.closing_time} | ${sh.total_orders} orders | Rs ${sh.total_sales}`);

      // Track Shiv's shift (floor 39, cashier 205)
      if (sh.cashier_id === 205) {
        shivShiftId = shResult.insertId;
      }
    }

    // ══════════════════════════════════════════════════════════════
    // 5d. Insert ZOMATO orders as outside_collections in Shiv's shift
    // ══════════════════════════════════════════════════════════════
    console.log('\n  Inserting Zomato outside_collections in Shiv\'s shift...');
    const insertedOCIds = [];
    ...
    for (const oc of outsideCollections) {
      ...
      const [ocResult] = await conn.query(
        `INSERT INTO outside_collections (
          uuid, outlet_id, shift_id, floor_id, collected_by,
          amount, payment_mode, reason, description,
          collection_date, status, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?,
                  ?, ?, ?, ?,
                  ?, 'active', ?, ?)`,
        [
          ocUuid, OUTLET_ID, shivShiftId, oc.floor_id, oc.collected_by,
          oc.amount, oc.payment_mode, oc.reason, oc.description,
          oc.collection_date, oc.createdAt, oc.createdAt
        ]
      );
      insertedOCIds.push(ocResult.insertId);
      console.log(`    OC #${ocResult.insertId} | Rs ${oc.amount} | shift_id=${shivShiftId} | ${oc.description.substring(0, 60)}`);
    }

    // ══════════════════════════════════════════════════════════════
    // 5e. VERIFY BEFORE COMMIT
    // ══════════════════════════════════════════════════════════════
    console.log('\n  Verifying...');
    ...
    const [verifyOC] = await conn.query(
      `SELECT COUNT(*) as cnt, SUM(amount) as total FROM outside_collections WHERE id IN (${insertedOCIds.length > 0 ? insertedOCIds.join(',') : '0'}) AND status = 'active'`
    );
    ...
    console.log(`    Outside Collections: ${verifyOC[0].cnt} inserted, Rs ${r2(verifyOC[0].total)} (expected: ${outsideCollections.length}, Rs ${r2(totalZomato)})`);
    ...
    if (parseInt(verifyOC[0].cnt) !== outsideCollections.length) {
      throw new Error(`OC count mismatch: ${verifyOC[0].cnt} vs ${outsideCollections.length}`);
    }
    ...
    console.log(`  Outside Collection IDs: ${insertedOCIds.join(', ')}`);
    ...
    const [bdTotals] = await conn.query(
      `SELECT COUNT(*) as cnt, SUM(total_amount) as sale, SUM(discount_amount) as discount, SUM(tax_amount) as tax
       FROM orders WHERE outlet_id = ? AND status = 'completed'
         AND created_at >= '${DATE_STR} 00:00:00' AND created_at < '2026-04-10 04:00:00'`,
      [OUTLET_ID]
    );
    console.log(`\n  Apr 9 (completed orders):`);
    ...
    const [floorTotals] = await conn.query(
      `SELECT COALESCE(f.name, 'Takeaway') as name, COUNT(*) as cnt, SUM(o.total_amount) as sale
       ...
       WHERE o.outlet_id = ? AND o.status = 'completed'
         AND o.created_at >= '${DATE_STR} 00:00:00' AND o.created_at < '2026-04-10 04:00:00'
       GROUP BY f.name`,
      [OUTLET_ID]
    );
    console.log('\n  By floor:');
    ...
    // Zomato outside collections
    const [ocTotals] = await conn.query(
      `SELECT COUNT(*) as cnt, SUM(amount) as total FROM outside_collections
       WHERE outlet_id = ? AND collection_date = ? AND status = 'active'`,
      [OUTLET_ID, DATE_STR]
    );
    console.log(`\n  Outside Collections (Zomato):`);
    console.log(`    Count: ${ocTotals[0].cnt}, Total: Rs ${r2(ocTotals[0].total)}`);
